...
for nr in sone_nrs:
    c_gk = grunnkretser[
        grunnkretser["grunnkretsnummer"].apply(lambda x: x[-4:-2] == nr)
    ]
    ...
    soner = pd.concat(
        [soner, gpd.GeoDataFrame({"sone": nr}, geometry=boundary, index=[index])]
    )
    index = index + 1
...
dft["price_indexed"] = dft["price_inc_debt"] * PI.reindex(
    datetime.date.today(), dft["adcreated_date"]
)

dft["buildyear_cat"] = (
    dft["buildyear"]
    .apply(lambda x: +1 * (x < 2015) + 1 * (x < 2000) + 1 * (x < 1980) + 1 * (x < 1950))
    .fillna(4)
)
dft["floor_cat"] = (
    dft["floor"]
    .apply(lambda x: +1 * (x < 1) + 1 * (x < 2) + 1 * (x < 4) + 1 * (x < 8))
    .fillna(1)
)
dft["size_cat"] = (
    dft["PROM"]
    .astype("float")
    .apply(lambda x: int(np.maximum(np.minimum(np.round(x / 10), 11), 2)))
)
...
for ii in grunnkretser.index:
    ind = df_points[
        df_points["geometry"].within(grunnkretser["geometry"].loc[ii])
    ].index
    df_points.loc[ind, "grunnkrets_id"] = ii
    if ii % 50 == 0:
        print(ii)

df_points = df_points.join(
    grunnkretser[["grunnkretsnummer", "grunnkretsnavn"]], on="grunnkrets_id"
)
...
date_interp = pd.Series(
    pd.date_range(start="2016-1-01", end="2021-6-30", freq="Q").date
)
...
for ii, gk_ind in enumerate(grunnkretser.index):
    ...
    c_price = df2.loc[ind][["price_res", "adcreated_date"]].sort_values(
        "adcreated_date"
    )
    ...
    sparklines_mat[gk_ind, :] = np.interp(
        (date_interp - d0).apply(lambda x: x.days),
    ...
    rate_res_price[gk_ind] = (
        lm.coef_ * (1e9 * 60 * 60 * 24 * 365) * 100
